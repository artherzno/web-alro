...
class DebtAgePerCodeTab extends React.Component {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.getDebtAgeByCode(parameter).then(response => {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.exportDebtAgePerCode(parameter).then(response => {

            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'รายงานอายุหนี้(รายประเภทโครงการหลัก).xlsx');
    ...
    printPDF() {
        ...
        api.getDebtAgeByCodePdf(parameter).then(response => {
    ...
    render() {
        ...
        return (<div>
            ...
            <div>
                <Box mt={1} mb={5}>
                    <Typography variant="h6" align="center">รายงานอายุหนี้ (รายประเภทโครงการหลัก) {`${this.state.provinceZoneLabel}`}</Typography>
...
export default withStyles(styles)(DebtAgePerCodeTab)