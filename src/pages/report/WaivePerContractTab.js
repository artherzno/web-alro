...
class WaivePerContractTab extends React.Component {
    ...
    loadPayLoan(page, count) {
        ...
        api.getWaivePerContract(parameter).then(response => {
    ...
    exportExcel() {
        ...
        api.exportWaivePerContract(parameter).then(response => {

            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'รายงานการขอผ่อนผัน.xlsx');
    ...
    render() {
        ...
        return (<div>
            ...
            <div>
                <Box mt={1} mb={5}>
                    <Typography variant="h6" align="center">รายงานการขอผ่อนผัน {`${this.state.provinceZoneLabel}`}</Typography>
...
export default withStyles(styles)(WaivePerContractTab)