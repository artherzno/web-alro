...
class DebtStatusPerLoanTypeTab extends React.Component {
    ...
    loadPayLoan(page, count) {
        ...
        api.getDebtStatusPerLoanType(parameter).then(response => {
    ...
    exportExcel() {
        ...
        api.exportDebtStatusPerLoanType(parameter).then(response => {

            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'รายงานสถานะหนี้เงินกู้ ระยะสั้น ระยะยาว.xlsx');
    ...
    printPDF() {
        ...
        api.getDebtStatusPerLoanTypePdf(parameter).then(response => {
    ...
    render() {
        ...
        return (<div>
            ...
            <div>
                <Box mt={1} mb={5}>
                    <Typography variant="h6" align="center">รายงานสถานะหนี้เงินกู้ ระยะสั้น ระยะยาว {`${this.state.provinceZoneLabel}`}</Typography>
...
export default withStyles(styles)(DebtStatusPerLoanTypeTab)