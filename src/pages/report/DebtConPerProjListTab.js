...
class DebtConPerProjListTab extends React.Component {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.getDebtConPerProList(parameter).then(response => {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.exportDebtConPerProjList(parameter).then(response => {

            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'รายงานสภาพหนี้(รายโครงการ).xlsx');
    ...
    render() {
        ...
        return (<div>
            ...
            <div>
                <Box mt={1} mb={5}>
                    <Typography variant="h6" align="center">รายงานสภาพหนี้ (รายโครงการ) {`${this.state.provinceZoneLabel}`}</Typography>
...
export default withStyles(styles)(DebtConPerProjListTab)