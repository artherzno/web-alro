...
class ResultDebtsRemainingPerProvinceTab extends React.Component {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.getGetResultDebtsRemainingByProvince(parameter).then(response => {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.exportResultDebtsRemainingPerProvince(parameter).then(response => {

            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'รายงานผลจัดเก็บหนี้ค้างชำระ/หนี้คงเหลือ (รายจังหวัด).xlsx');
    ...
    printPDF() {
        ...
        api.getResultDebtsRemainingByProvincePdf(parameter).then(response => {
    ...
    render() {
        ...
        return (<div>
            ...
            <div>
                <Box mt={1} mb={5}>
                    <Typography variant="h6" align="center">รายงานผลจัดเก็บหนี้ค้างชำระ/หนี้คงเหลือ(รายจังหวัด) {`${this.state.provinceZoneLabel}`}</Typography>
                        ...
                        <Table className={classes.table} aria-label="customized table">
                            <TableHead>
                                <TableRow>
                                    ...
                                    <StyledTableCell align="center" colSpan={2}>หนี้คงเหลือ (เงินต้น)</StyledTableCell>
                                    <StyledTableCell align="center" colSpan={4}>หนี้ค้างชำระ</StyledTableCell>
...
export default withStyles(styles)(ResultDebtsRemainingPerProvinceTab)