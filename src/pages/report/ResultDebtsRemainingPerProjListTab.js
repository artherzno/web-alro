...
class ResultDebtsRemainingPerProjListTab extends React.Component {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.getGetResultDebtsRemainingByProject(parameter).then(response => {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.exportResultDebtsRemainingPerProjList(parameter).then(response => {

            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'รายงานผลจัดเก็บหนี้ค้างชำระ/หนี้คงเหลือ (รายโครงการ).xlsx');
    ...
    printPDF() {
        ...
        api.getResultDebtsRemainingByProjectPdf(parameter).then(response => {
    ...
    render() {
        ...
        return (<div>
            ...
            <div>
                <Box mt={1} mb={5}>
                    <Typography variant="h6" align="center">รายงานผลจัดเก็บหนี้ค้างชำระ/หนี้คงเหลือ (รายโครงการ) {`${this.state.provinceZoneLabel}`}</Typography>
...
export default withStyles(styles)(ResultDebtsRemainingPerProjListTab)