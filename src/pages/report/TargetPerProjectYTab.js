...
class TargetPerProjectYTab extends React.Component {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.getTargetByProjectY(parameter).then(response => {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.exportTargetByProjectY(parameter).then(response => {

            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'รายงานเป้าจัดเก็บรายโครงการ(ปี).xlsx');
    ...
    printPDF() {
        ...
        api.getTargetByProjectYearPdf(parameter).then(response => {
    ...
    render() {
        ...
        return (<div>
            ...
            <div>
                <Box mt={1} mb={5}>
                    <Typography variant="h6" align="center">รายงานเป้าจัดเก็บ รายโครงการ(ปี) {`${this.state.provinceZoneLabel}`}</Typography>
...
export default withStyles(styles)(TargetPerProjectYTab)