...
class ComparePerProjPlanTab extends React.Component {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.exportComparePerProjPlan(parameter).then(response => {

            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'รายงานสรุปเปรียบเทียบแผน/ผลจัดเก็บ(แผนโครงการ).xlsx');
    ...
    printPDF() {
        ...
        api.getComparePerCodePdf(parameter).then(response => {
    ...
    render() {
        ...
        return (<div>
            ...
            <div>
                <Box mt={1} mb={5}>
                    <Typography variant="h6" align="center">รายงานสรุปเปรียบเทียบแผน / ผล จัดเก็บ (แผนโครงการ) {`${this.state.provinceZoneLabel}`}</Typography>
...
export default withStyles(styles)(ComparePerProjPlanTab)