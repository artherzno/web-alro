...
class DebtAgePerProvinceTab extends React.Component {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.getDebtAgeByProvince(parameter).then(response => {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.exportDebtAgePerProvince(parameter).then(response => {

            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'รายงานอายุหนี้(รายจังหวัด).xlsx');
    ...
    printPDF() {
        ...
        api.getDebtAgeByProvincePdf(parameter).then(response => {
    ...
    render() {
        ...
        return (<div>
            ...
            <div>
                <Box mt={1} mb={5}>
                    <Typography variant="h6" align="center">รายงานอายุหนี้ (รายจังหวัด) {`${this.state.provinceZoneLabel}`}</Typography>
...
export default withStyles(styles)(DebtAgePerProvinceTab)