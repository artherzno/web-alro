...
class LiabilityPerContractTab extends React.Component {
    ...
    loadPayLoan(page, count) {
        ...
        api.getLiabilityPerContract(parameter).then(response => {
    ...
    exportExcel() {
        ...
        api.exportLiabilityPerContract(parameter).then(response => {

            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'รายงานสภาพความรับผิด.xlsx');
    ...
    render() {
        ...
        return (<div>
            ...
            <div>
                <Box mt={1} mb={5}>
                    <Typography variant="h6" align="center">รายงานสภาพความรับผิด {`${this.state.provinceZoneLabel}`}</Typography>
...
export default withStyles(styles)(LiabilityPerContractTab)