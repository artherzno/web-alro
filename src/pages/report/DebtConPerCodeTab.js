...
class DebtConPerCodeTab extends React.Component {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.exportDebtConPerCode(parameter).then(response => {

            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'รายงานสภาพหนี้(รายประเภทโครงการหลัก).xlsx');
    ...
    render() {
        ...
        return (<div>
            ...
            <div>
                <Box mt={1} mb={5}>
                    <Typography variant="h6" align="center">รายงานสภาพหนี้ (รายประเภทโครงการหลัก) {`${this.state.provinceZoneLabel}`}</Typography>
...
export default withStyles(styles)(DebtConPerCodeTab) 