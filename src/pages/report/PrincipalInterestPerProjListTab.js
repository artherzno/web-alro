...
class PrincipalInterestPerProjListTab extends React.Component {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.getPrincipalInterestByProject(parameter).then(response => {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.exportPrincipalInterestPerProjList(parameter).then(response => {

            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'รายงานหนี้เงินต้นและดอกเบี้ยค้างชำระ (รายโครงการ).xlsx');
    ...
    printPDF() {
        ...
        api.getPrincipalInterestByProjectPdf(parameter).then(response => {
    ...
    render() {
        ...
        return (<div>
            ...
            <div>
                <Box mt={1} mb={5}>
                    <Typography variant="h6" align="center">รายงานหนี้เงินต้นและดอกเบี้ยค้างชำระ  (รายโครงการ) {`${this.state.provinceZoneLabel}`}</Typography>
...
export default withStyles(styles)(PrincipalInterestPerProjListTab)