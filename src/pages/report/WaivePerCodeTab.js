...
class WaivePerCodeTab extends React.Component {
    ...
    loadPayLoan(page, count) {
        ...
        api.getWaivePerCode(parameter).then(response => {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.exportWaivePerCode(parameter).then(response => {

            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'รายงานการขอผ่อนผัน(รายประเภทโครงการหลัก).xlsx');
    ...
    render() {
        ...
        return (<div>
            ...
            <div>
                <Box mt={1} mb={5}>
                    <Typography variant="h6" align="center">รายงานการขอผ่อนผัน (รายประเภทโครงการหลัก) {`${this.state.provinceZoneLabel}`}</Typography>
...
export default withStyles(styles)(WaivePerCodeTab) 