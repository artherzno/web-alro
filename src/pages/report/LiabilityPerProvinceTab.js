...
class LiabilityPerProvinceTab extends React.Component {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.getLiabilityPerProvince(parameter).then(response => {
    ...
    exportExcel() {
        ...
        api.exportLiabilityPerProvince(parameter).then(response => {

            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'สรุปรายงานสภาพความรับผิด.xlsx');
    ...
    render() {
        ...
        return (<div>
            ...
            <div>
                <Box mt={1} mb={5}>
                    <Typography variant="h6" align="center">สรุปรายงานสภาพความรับผิด {`${this.state.provinceZoneLabel}`}</Typography>
...
export default withStyles(styles)(LiabilityPerProvinceTab)