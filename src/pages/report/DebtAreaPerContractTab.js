...
class DebtAreaPerContractTab extends React.Component {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.getDebtAreaPerContract(parameter).then(response => {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.exportDebtAreaPerContract(parameter).then(response => {

            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'รายงานการออกจัดเก็บหนี้ในพื้นที่.xlsx');
    ...
    printPDF() {
        ...
        api.getDebtAreaPerContractPdf(parameter).then(response => {
    ...
    render() {
        ...
        return (<div>
            ...
            <div>
                <Box mt={1} mb={5}>
                    <Typography variant="h6" align="center">รายงานการออกจัดเก็บหนี้ในพื้นที่ {`${this.state.provinceZoneLabel}`}</Typography>
...
export default withStyles(styles)(DebtAreaPerContractTab)