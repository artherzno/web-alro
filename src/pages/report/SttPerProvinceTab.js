...
class SttPerProvinceTab extends React.Component {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.getSttByProvince(parameter).then(response => {
...
parameter.append('YearTo', YearTovalue);
        ...
        api.exportSttPerProvince(parameter).then(response => {

            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'รายงานสถานสภาพหนี้สิ้น(จังหวัด).xlsx');
    ...
    printPDF() {
        ...
        api.getSttByProvincePdf(parameter).then(response => {
    ...
    render() {
        ...
        return (<div>
            ...
            <div>
                <Box mt={1} mb={5}>
                    <Typography variant="h6" align="center">รายงานสถานสภาพหนี้สิ้น (จังหวัด) {`${this.state.provinceZoneLabel}`}</Typography>
...
export default withStyles(styles)(SttPerProvinceTab)