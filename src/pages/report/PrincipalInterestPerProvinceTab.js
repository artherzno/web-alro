...
class PrincipalInterestPerProvinceTab extends React.Component {
    ...
    loadPayLoan(page, count) {
        ...
        api.getPrincipalInterestByProvince(parameter).then(response => {
    ...
    exportExcel() {
        ...
        api.exportPrincipalInterestPerProvince(parameter).then(response => {

            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'รายงานหนี้เงินต้นและดอกเบี้ยค้างชำระ (รายจังหวัด).xlsx');
    ...
    printPDF() {
        ...
        api.getPrincipalInterestByProvincePdf(parameter).then(response => {
    ...
    render() {
        ...
        return (<div>
            ...
            <div>
                <Box mt={1} mb={5}>
                    <Typography variant="h6" align="center">รายงานหนี้เงินต้นและดอกเบี้ยค้างชำระ (รายจังหวัด) {`${this.state.provinceZoneLabel}`}</Typography>
...
export default withStyles(styles)(PrincipalInterestPerProvinceTab)