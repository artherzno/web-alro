...
class ComparePlanResultPerContractTab extends React.Component {
    ...
    loadPayLoan(page, count) {
        ...
        api.getComparePlanResultByContract(parameter).then(response => {
    ...
    exportExcel() {
        ...
        api.exportRequestLoan(parameter).then(response => {
            ...
            link.href = url;
            link.setAttribute('download', 'รายงานเปรียบเทียบแผน-ผลการจ่ายเงินกู้.xlsx');
    ...
    printPDF() {
        ...
        api.getComparePlanResultByContractPdf(parameter).then(response => {
    ...
    render() {
        ...
        return (<div>
            ...
            <div>
                <Box mt={1} mb={5}>
                    <Typography variant="h6" align="center">รายงานเปรียบเทียบแผน-ผลการจ่ายเงินกู้ {`${this.state.provinceZoneLabel}`}</Typography>
...
export default withStyles(styles)(ComparePlanResultPerContractTab)