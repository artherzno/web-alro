...
function DebtConditionAdjust () {
    ...
    const rowsLabel = [
        'ConditionCode', 'ConditionName', 'ConditionDetail'
    ]

    const headCells = [
        { id: 'ConditionCode', numeric: false, disablePadding: true, widthCol: '60px', label: 'รหัสเงื่อนไข' },
        { id: 'ConditionName', numeric: false, disablePadding: true, widthCol: '160px', label: 'ชื่อเงื่อนไข' },
        { id: 'ConditionDetail', numeric: false, disablePadding: true, widthCol: '160px', label: 'Cd_desc' },
    ]

    function createData(ConditionCode, ConditionName, ConditionDetail) {
        return {ConditionCode, ConditionName, ConditionDetail}
    }
    ...
    useEffect(() => {
        ...
        let data = require('../../datamockup/Condition_JSON.json'); 
        let dataArr = []
        for (var i = 0; i < data.length; i++) {
            var obj = data[i];
            // console.log(`Name: ${obj.DistrictID}, ${obj.AM_NAME}`);
            dataArr.push(createData(obj.ConditionCode, obj.ConditionName, obj.ConditionDetail))
        }
        setRows(dataArr)
    }, [])
                ...
                <div className="fade">
                    <Container maxWidth="md">
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12} md={3}>
                                <MuiTextfield label="ชื่อเงื่อนไข" name="Code" value={inputDataSearch.Code} onChange={handleInputDataSearch} />
                            </Grid>
                            <Grid item xs={12} md={3}>
                                <p>&nbsp;</p>
...
export default DebtConditionAdjust
